...
serve(async (req) => {
  ...
  const androidIntent = `intent://auth?reset=true&token=${encodeURIComponent(token)}&type=${encodeURIComponent(type)}#Intent;scheme=dialroad;package=${androidPackage};S.browser_fallback_url=${encodeURIComponent(webTarget)};end`;
